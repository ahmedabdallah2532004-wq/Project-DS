...
async function ensureTopic() {
  ...
  try {
    const topicsToCreate = [
      ...
      topics.FILE_DOWNLOADED,
      topics.FILE_SHARED,
      topics.PREVIEW_REQUESTED
    ];
      ...
      await consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
          try {
            ...
            // Process event (e.g., store analytics)
            logger.info(`Access Analytics Service processing event of type ${event.type}`);
