...
const backupProducer = require('./backupProducer');
      ...
      await consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
          try {
            ...
            logger.info(`Backup Service starting backup for request ${event.id}`);
            await sleep(2000); // Simulate work

            // Produce backup.completed
            await backupProducer.publishBackupEvent(topics.BACKUP_COMPLETED, {
              backup_id: event.id,
              status: 'success',
...
async function stop() {
  ...
  await backupProducer.disconnect();
}
