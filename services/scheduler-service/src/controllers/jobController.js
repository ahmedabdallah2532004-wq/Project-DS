...
const runPlatformJob = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      ...
      status: 'completed',
      tasks: {
        backup_cleanup: jobType === 'backup-cleanup' || jobType === 'manual' ? 'triggered' : 'skipped',
        metrics_aggregation: jobType === 'metrics-aggregation' || jobType === 'manual' ? 'triggered' : 'skipped',
        analytics_summary: jobType === 'analytics-summary' || jobType === 'manual' ? 'triggered' : 'skipped'
      }
    });
  } catch (error) {
    next(error);
...
const getPlatformJobStatus = async (req, res, next) => {
  try {
    res.status(200).json({
      status: 'ready',
      scheduled_jobs: ['backup-cleanup', 'metrics-aggregation', 'analytics-summary']
    });
  } catch (error) {
    next(error);
  }
...
module.exports = { createNewJob, runJobDirectly, getAllJobs, runPlatformJob, getPlatformJobStatus };
