...
describe('scheduler service integration routes', () => {
  ...
  test('GET /api-docs redirects to docs', async () => {
    await request(app).get('/api-docs').expect(302);
  });
  ...
  test('POST /jobs/run executes platform scheduled work', async () => {
    ...
    expect(response.body.tasks.backup_cleanup).toBe('triggered');
  });
});
